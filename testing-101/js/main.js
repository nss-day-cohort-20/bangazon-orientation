...
module.exports.addCustomer = ({first_name, last_name, street, city, state, zip, phone}) => {
  return new Promise( (resolve, reject) => {
    db.run(`INSERT into customers VALUES(null, "${first_name}", "${last_name}", "${street}", "${city}", "${state}", "${zip}", "${phone}")`, function(err) {
      if(err) return reject(err)
      resolve({msg: "New customer added", custId: this.lastID});
    });
  });
};
